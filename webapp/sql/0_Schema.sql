...
CREATE TABLE `isu` (
  ...
  PRIMARY KEY(`id`),
  KEY `by_jia_isu_uuid` (`jia_isu_uuid`),
  KEY `by_jia_user_id_and_jia_isu_uuid` (`jia_user_id`, `jia_isu_uuid`)
...
CREATE TABLE `isu_condition` (
  ...
  `is_dirty` TINYINT(1),
  `is_overweight` TINYINT(1),
  `is_broken` TINYINT(1),
  `condition_level` ENUM('info', 'warning', 'critical'),
  `condition` VARCHAR(255) NOT NULL,
  `message` VARCHAR(255) NOT NULL,
  `created_at` DATETIME(6) DEFAULT CURRENT_TIMESTAMP(6),
  PRIMARY KEY(`id`),
  KEY `by_jia_isu_uuid_and_timestamp` (`jia_isu_uuid`, `timestamp`)
) ENGINE=InnoDB DEFAULT CHARACTER SET=utf8mb4;
